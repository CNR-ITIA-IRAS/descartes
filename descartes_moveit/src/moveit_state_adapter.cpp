                                    ...
                                    const std::string& world_frame, const std::string& tcp_frame)
{
  ...
  if (!model)
  {
    ROS_ERROR("Failed to load robot model from robot description parameter: %s", robot_description.c_str());
    return false;
  }
                                    ...
                                    const std::string &world_frame, const std::string &tcp_frame)
{
  ...
  {
    ROS_ERROR("%s: Tool frame '%s' does not match group tool frame '%s', functionality"
  ...
  {
    ROS_WARN("%s: Could not determine velocity limits of RobotModel from MoveIt", __FUNCTION__);
  }
  ...
  {
    ROS_INFO("%s: World frame '%s' does not match model root frame '%s', all poses will be"
...
bool MoveitStateAdapter::getIK(const Eigen::Affine3d& pose, std::vector<double>& joint_pose) const
{
  ...
  {
    ROS_ERROR("somethings strange: %s",e.what());
    return false;
  }
...
bool MoveitStateAdapter::getAllIK(const Eigen::Affine3d& pose, std::vector<std::vector<double> >& joint_poses) const
{
  ...
  ROS_DEBUG("Utilizing an min. difference of %f between IK solutions", epsilon);
  joint_poses.clear();
    ...
    if (getIK(pose, joint_pose))
    {
      ...
      else
      {
        ...
        for (joint_pose_it = joint_poses.begin(); joint_pose_it != joint_poses.end(); ++joint_pose_it)
        {
          ...
          {
            ROS_DEBUG("Found matching, potential solution is not new");
            match_found = true;
            break;
          }
        }
  ...
  ROS_DEBUG("Found %lu joint solutions out of %lu iterations", static_cast<unsigned long>(joint_poses.size()),
           static_cast<unsigned long>(seed_states_.size()));

  if (joint_poses.empty())
  {
    ROS_ERROR("Found 0 joint solutions out of %lu iterations", static_cast<unsigned long>(seed_states_.size()));
    return false;
  }
  else
  {
    ROS_INFO("Found %lu joint solutions out of %lu iterations", static_cast<unsigned long>(joint_poses.size()),
              static_cast<unsigned long>(seed_states_.size()));
    return true;
  }
}
...
bool MoveitStateAdapter::getFK(const std::vector<double>& joint_pose, Eigen::Affine3d& pose) const
{
  ...
  if (isValid(joint_pose))
  {
    ...
    {
      ROS_ERROR("Robot state does not recognize tool frame: %s", tool_frame_.c_str());
      rtn = false;
    }
  }
  else
  {
    ROS_ERROR("Invalid joint pose passed to get forward kinematics");
    rtn = false;
  }
...
bool MoveitStateAdapter::isValid(const std::vector<double>& joint_pose) const
{
  ...
  {
    ROS_ERROR("Size of joint pose: %lu doesn't match robot state variable size: %lu",
