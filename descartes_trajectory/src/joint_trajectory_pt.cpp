...
#define NOT_IMPLEMENTED_ERR(ret)                                                                                       \
  ROS_ERROR("%s not implemented", __PRETTY_FUNCTION__);                                                                 \
  return ret;
...
bool JointTrajectoryPt::setDiscretization(const std::vector<double> &discretization)
{
  ...
  {
    ROS_ERROR("discretization must be size 1 or same size as joint count.");
    return false;
  }
  ...
  for (size_t ii = 0; ii < discretization.size(); ++ii)
  {
    ...
    {
      ROS_ERROR("discretization value out of range.");
      return false;
    }
  }
